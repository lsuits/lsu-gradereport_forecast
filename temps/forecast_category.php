...
class forecast_category extends grade_category {
    ...
    /**
     * Instantiate a new instance given a grade_category_id
     * 
     * @param  int  $grade_category_id
     * @return forecast_category
     */
    public static function findByGradeCategoryId($grade_category_id) {
        $grade_item = grade_item::fetch([
            'itemtype' => 'category',
            'iteminstance' => $grade_category_id,
        ]);

        if ( ! $grade_item or ! property_exists($grade_item, 'id')) {
            return false;
        }

        return new self($grade_item->id);
    }
